<template>
  <div class="max-w-6xl mx-auto px-4 py-8">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="vehiculo in vehiculos" :key="vehiculo.id" class="bg-white rounded-lg shadow-md flex flex-col overflow-hidden border border-gray-200">
          ...
          <div class="p-4 flex flex-col flex-grow">
            ...
            <button
              class="mt-4 w-full bg-gray-100 text-gray-700 py-2 rounded hover:bg-gray-200 font-medium transition"
              @click="vehiculoSeleccionado = vehiculo"
            >
              Ver Detalles
            </button>
          </div>
    ...
    <!-- Modal Ver Detalles -->
    <div v-if="vehiculoSeleccionado" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
        <img
          :src="vehiculoSeleccionado.foto || 'https://via.placeholder.com/600x300?text=Sin+Foto'"
          :alt="vehiculoSeleccionado.patente"
          class="w-full h-56 object-cover"
        >
        <div class="p-6">
          <div class="flex justify-between items-start mb-4">
            <h2 class="text-2xl font-bold text-blue-900 uppercase">{{ vehiculoSeleccionado.patente }}</h2>
            <span
              class="text-xs font-bold rounded-full px-3 py-1"
              :class="{
                'bg-green-100 text-green-800': vehiculoSeleccionado.estado === 'disponible',
                'bg-blue-100 text-blue-800': vehiculoSeleccionado.estado === 'arrendado',
                'bg-yellow-100 text-yellow-800': vehiculoSeleccionado.estado === 'en mantenimiento',
                'bg-red-100 text-red-800': vehiculoSeleccionado.estado === 'de baja'
              }"
            >
          ...
          <div class="space-y-2 text-sm text-gray-700">
            <p><span class="font-semibold">Marca:</span> {{ vehiculoSeleccionado.marca }}</p>
            <p><span class="font-semibold">Modelo:</span> {{ vehiculoSeleccionado.modelo }}</p>
            <p><span class="font-semibold">Tipo:</span> {{ vehiculoSeleccionado.tipo?.nombre }}</p>
            <p><span class="font-semibold">Valor diario:</span> ${{ vehiculoSeleccionado.tipo?.valorDia?.toLocaleString('es-CL') }}</p>
          </div>
          <button
            class="mt-6 w-full bg-gray-100 text-gray-700 py-2 rounded hover:bg-gray-200 font-medium"
            @click="vehiculoSeleccionado = null"
          >
            Cerrar
          </button>
        </div>
